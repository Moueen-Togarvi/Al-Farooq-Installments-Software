...
<div class="space-y-8">
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
		{#each statsCards as stat}
			...
			<div class="bg-white p-6 rounded-xl border-2 border-gray-200 shadow-sm flex flex-col justify-between group">
				<div class="flex items-center justify-between">
					<p class="text-xs font-black text-gray-500 uppercase tracking-widest">{stat.name}</p>
					<div class="bg-gray-100 p-2 rounded-lg group-hover:bg-black transition-colors">
						<Icon class="w-5 h-5 text-gray-900 group-hover:text-white" />
					</div>
				</div>
				<div class="mt-5">
					<p class="text-3xl font-black text-black tracking-tight">{stat.value}</p>
				</div>
			</div>
		{/each}
	</div>
					...
					<div class="px-6 py-4 hover:bg-gray-50 transition-colors flex items-center justify-between group">
						<div class="flex items-center gap-4">
							...
							<div>
								...
								<p class="text-xs font-bold text-gray-500 mt-0.5">{item.plan?.product?.name || 'Unknown'}</p>
							</div>
						</div>
						<div class="text-right flex flex-col items-end">
							<p class="text-sm font-black text-black">{formatCurrency(item.amount)}</p>
							<a 
