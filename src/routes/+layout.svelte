...
{:else}
	<div class="min-h-screen bg-gray-50 flex font-sans">
		...
		<!-- Sidebar -->
		<aside 
			class="fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 z-50 transform transition-transform duration-300 lg:translate-x-0 {isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}"
		>
		...
		<div class="flex-1 flex flex-col lg:pl-64">
			<!-- Top Header -->
			<header class="h-16 bg-white border-b border-gray-200 sticky top-0 z-30 flex items-center justify-between px-6 lg:px-8">
				<div class="flex items-center gap-4">
					<button 
						class="lg:hidden p-2 -ml-2 rounded-lg text-gray-600 hover:bg-gray-50 transition-colors"
						onclick={toggleSidebar}
